...
class Mnist():
    ...
    def read_image(self, path):
        with gzip.open(path, "rb") as f:
            ...
            image = cp.frombuffer(f.read(), dtype = cp.uint8).reshape(num, rows, cols)
        return image
    
    def read_label(self, path):
        with gzip.open(path, "rb") as f:
            magic, num = unpack(">2I", f.read(8))
            label = cp.frombuffer(f.read(), dtype = cp.uint8)
        return label
    ...
    def load(self, rate, normalized=True):
        ...
        tot_idx = cp.arange(train_image.shape[0])
        cp.random.shuffle(tot_idx)
