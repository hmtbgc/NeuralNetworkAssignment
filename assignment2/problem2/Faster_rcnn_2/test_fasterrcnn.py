...
sys.path.append("E:\\ZA\\ZA\\NN\\Faster-rcnn")
...
def prepare_config():
    ...
    cfg.merge_from_file("Faster-rcnn/configs/config.yaml")
    cfg.freeze()
...
def main(cfg):
    ...
    draw_proposal = False
    model_path = Path(cfg.MODEL.saved_path) / cfg.MODEL.name / 'model.pt'
    ...
    for i, data in enumerate(tqdm(test_loader)):
        ...
        if not draw_proposal:
            if len(outputs[0]['boxes']) != 0:
                ...
                draw_box(orig_image, draw_boxes, pred_classes, scores, CLASSES=CLASSES, COLORS=COLORS, label=True)
                cv2.waitKey(1)
                cv2.imwrite(f"Faster-rcnn/imgs/output/test/{test_label['image_name']}.jpg", orig_image)
                if i > 3:
                    break
        else:
            print(outputs)
            if True:
                ...
                # draw the bounding boxes and write the class name on top of it
                draw_box(orig_image, draw_boxes, None, sorted_score, CLASSES, COLORS, label=False)
                cv2.waitKey(1)
                cv2.imwrite(f"Faster-rcnn/imgs/output/proposal_box_test/{test_label['image_name']}.jpg", orig_image)
